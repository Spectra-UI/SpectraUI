[![Static Badge](https://img.shields.io/badge/Contribute-Patreon?style=for-the-badge&logo=patreon&logoColor=FF424D&color=151B23)](https://www.patreon.com/SpectraUI)
[![CurseForge Version](https://img.shields.io/curseforge/v/1102865?style=for-the-badge&logo=curseforge&logoColor=F16436&label=Version&labelColor=151B23&color=2AB6FF)](https://www.curseforge.com/wow/addons/spectraui)

<details>
  
<summary><strong>UI Screenshots (Click to Expand)</strong></summary>

<img width="3439" height="1439" alt="Screenshot 2026-02-09 023451" src="https://github.com/user-attachments/assets/ca127625-e646-4558-bc98-8611c4f238cb" />
<img width="3439" height="1439" alt="Screenshot 2026-02-09 024059" src="https://github.com/user-attachments/assets/dbd45d90-8b6a-4b99-8d06-00051a6a4767" />
<img width="3439" height="1439" alt="Screenshot 2026-02-09 024239" src="https://github.com/user-attachments/assets/9fd98356-1b46-4460-a830-b131d93072c5" />

</details>

---

<img width="836" height="100" alt="Features" src="https://github.com/user-attachments/assets/3fc591b9-2352-46d0-a868-55c7fc5cf2e5" />

- High-end, performance-focused UI profiles
- Custom tags for enhanced information display
- Unified texture and font packs
- Custom designed icons across the UI
- Streamlined installer for quick setup
- Active development and community support via Discord
- And much more!

---
<img width="836" height="100" alt="Requirements" src="https://github.com/user-attachments/assets/2fc87a47-ef4f-405a-aa0a-419cc066b52d" />

## Required AddOns
- [**ElvUI**](https://tukui.org/elvui)
- [**WeakAuras**](https://www.curseforge.com/wow/addons/weakauras-2)

## Recommended AddOns
- [**Details**](https://www.curseforge.com/wow/addons/details-damage-meter-burning-crusade)  
  *Damage meter*
- [**BigWigs**](https://www.curseforge.com/wow/addons/big-wigs)  
  *Boss encounter timers*
- [**ActionBar Buddy**](https://www.curseforge.com/wow/addons/actionbar-buddy-elvui-plugin)  
  *Inherits global fade settings*
- [**Leatrix Plus**](https://www.curseforge.com/wow/addons/leatrix-plus)  
  *General quality-of-life improvements*
- [**Leatrix Maps**](https://www.curseforge.com/wow/addons/leatrix-maps)  
  *World map enhancements*
- [**Baganator**](https://www.curseforge.com/wow/addons/baganator)  
  *Bag management*
- [**Auctionator**](https://www.curseforge.com/wow/addons/auctionator)  
  *Auction house tools & tracking*
- [**Chattynator**](https://www.curseforge.com/wow/addons/chattynator)  
  *Stylish chat frame*

## Clean High Quality Icons
- [**High Quality Icons (Clean Icons – Mechagnome Edition)**](https://www.wowinterface.com/downloads/info25064-CleanIcons-MechagnomeEdition.html)

---
<img width="836" height="100" alt="Installation" src="https://github.com/user-attachments/assets/19e89529-a185-49cd-b0ed-821af408460a" />

## Step 1: Backup Your WTF Folder

Before installing **SpectraUI**, it is highly recommended to start with a clean profile.

1. Go to your **World of Warcraft** installation folder.
2. Locate the **WTF** folder:
   - **Classic ERA / Anniversary:** ```\World of Warcraft\_Classic_Era_\WTF```
   - **MoP Classic:** ```\World of Warcraft\_classic_\WTF```
3. Copy and **store a backup** of this folder somewhere safe.

## Step 2: Enter the Game & Check AddOns

1. Launch **World of Warcraft**.
2. At the **Character Selection** screen, click **AddOns**.
3. Ensure all **required and recommended addons** are **enabled**.
4. If **SpectraUI** does not appear, enable **“Load out-of-date AddOns”**.

## Step 3: Run the SpectraUI Installer

Once inside the game:

1. The **SpectraUI installer** will automatically appear.
2. **Follow the on-screen instructions** carefully.
3. Adjust UI settings as desired.

> **If the installer does not appear, you can start it manually by typing:**  
> `/sui install` **in chat**

---

## Final Steps & Troubleshooting

If everything looks good — you’re all set. Enjoy your new **SpectraUI**!

### If you run into issues:
- Join the [**Discord**](https://discord.gg/UxHZRpSwQz) and open a **support ticket** in the appropriate channel for your game version.
- Run **`/reloadui`** after installation or any configuration changes.
- Double-check that all **required addons** are installed and loaded correctly.
---
<img width="836" height="100" alt="Credits" src="https://github.com/user-attachments/assets/59154d86-fdbb-4db9-8d76-0edd69d9aef0" />

- **Blinkii** for developing the initial **SpectraUI** codebase
- **Dlarge** German (DE) localization  
- **ZamestoTV** Russian (RU) localization
- **Lillekatt** for those lovely role icons
- **mMediaTag** textures and icon assets used in **SpectraUI**
- **Jiberish** & his community for the inspiration
- **ElvUI Discord** — Continuous help and support
- **Patreon supporters** — Your support helps keep **SpectraUI** evolving
---
<img width="836" height="100" alt="Discord" src="https://github.com/user-attachments/assets/6164ca3f-a824-4eb4-abe6-1024f1bb76c7" />

[![Discord](https://img.shields.io/discord/1283555480085729382?style=for-the-badge&logo=discord&logoColor=5865F2&label=Discord&labelColor=151B23)](https://discord.gg/UxHZRpSwQz)

### Join the **SpectraUI Discord** for:
- Support and troubleshooting
- Development updates and previews
- Community feedback and discussion

> This is the primary place for help and announcements.

---
